...
int main()
{
    ...
    while(t--){
      ...
      ll m;
      cin>>m;
      vector<ll>prefup(m),suffdown(m),vec(m);
      ll totup=0,totd=0;
      for(ll i=0;i<m;i++){
        cin>>vec[i];
        totup+=vec[i];
      ...
      prefup[0]=vec[0];
     for(ll i=1;i<m;i++){
        prefup[i]=prefup[i-1]+vec[i];
      }
     for(ll i=0;i<m;i++){
        cin>>vec[i];
        totd+=vec[i];
      }
      suffdown[m-1]=vec[m-1];
      for(ll i=m-2;i>=0;i--){
        suffdown[i]=suffdown[i+1]+vec[i];
      }
      ll ans=INT_MAX;
   
      for(ll i=0;i<m;i++){
        ans=min(ans,max((totup-prefup[i]),(totd-suffdown[i])));    
      }
      cout<<ans<<endl;
    }
