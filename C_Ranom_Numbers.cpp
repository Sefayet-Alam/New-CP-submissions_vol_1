...
ll dp[N][6][3];
vector<ll>powers(5);
ll func(ll i,ll lastmax,ll change){
    if(i==-1) return 0;
    if(dp[i][lastmax][change]!=-1) return dp[i][lastmax][change];
    ll sign=1;
    if(s[i]-'A'<lastmax) sign=-1;
   
    ll ans=sign*powers[s[i]-'A']+func(i-1,max(lastmax,(s[i]-'A')*1LL),change);
    if(!change){
        for(ll j=0;j<5;j++){
            sign=1;
            if(j<lastmax) sign=-1;
            ans=max(ans,sign*powers[j]+func(i-1,max(lastmax,j),1));
        }
    }
    return dp[i][lastmax][change]=ans;
}
