...
int main()
{
    ...
    while (t--)
    {
       ...
       vector<ll> vec(n),x(n), y(n), z(n + 1);
         map<ll, ll> freq, freq2, mpp;
        ...
        for (i = 0; i < n ; i++)
        {
             k = x[i];
             m = y[i];
            //  g = queryLazySegTree(0, 0, n, m, m);
             g=getSum(m+1);
            // ll g=querySegTree(0,0,n,m,m);

            // updateValSegTree(0,0,n,m,g-1);
            // updateLazySegTree(0, 0, n, m, m, -1);
            updateBIT(n+1,m+1,-1);

            // g=querySegTree(0,0,n,m,m);
            // cout<<g<<endl;
            // ll f = querySegTree(0, 0, n, 0, k - 1)
             f= getSum(k)-getSum(0);
            // queryLazySegTree(0,0,n,0,k-1);
            // cout<<k<<" "<<f<<endl;
            ans += f;
        }
        //   cout<<endl;
