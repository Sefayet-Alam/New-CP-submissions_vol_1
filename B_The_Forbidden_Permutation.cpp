...
int main()
{
    ...
    while(t--){
        ...
        //cout<<pos<<endl;
        for(ll i=1;i<m;i++){
            if(pos[vec[i-1]]>= pos[vec[i]] || pos[vec[i-1]]+d<pos[vec[i]]){
                ans=0;
                break;
                }
            else{
            ans=min(ans,abs(pos[vec[i]]-pos[vec[i-1]]));
            ll dist = pos[vec[i]]-pos[vec[i-1]];
            ll swapNeed = d-dist+1;
            ll swapPossible = (pos[vec[i-1]]-1) + (n - pos[vec[i]]);
