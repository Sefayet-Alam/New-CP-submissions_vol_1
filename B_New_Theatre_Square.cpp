...
ll func(ll i,ll j,ll tot){
   ...
   // ans=INT_MAX;
    ...
    if(dp[i][j]!=-1 && dp[i][j]<tot){
        return INT_MAX;
    }

          
    if(dp[i][j]!=-1) tot=min(tot,dp[i][j]);
    dp[i][j]=tot;

    
    
    //cout<<i<<" "<<j<<" "<<tot<<" "<<dp[i][j]<<endl;
    if(grid[i][j]=='.'){
        if(grid[i][j+1]=='.') ans=min(ans,func(i,j+2,tot+b));
        ans=min(ans,func(i,j+1,tot+a));



    }
    else ans=min(ans,func(i,j+1,tot));
