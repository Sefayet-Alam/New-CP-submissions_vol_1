...
int main()
{
    ...
    while(t--){
        ...
        //   cout<<maxm<<nn;
        //   cout<<maxm-vec[1]<<nn;
        ll ans1=(m-1)*(maxm-vec[1])+(n-1)*(m)*(maxm-vec[0]);
        // cout<<n-1<<" "<<m<<nn;
        ll ans2=(n-1)*(maxm-vec[1])+(m-1)*(n)*(maxm-vec[0]);
        ll ans3=(m-1)*(max2-minm)+(n-1)*(m)*(maxm-vec[0]);
        ll ans4=(n-1)*(max2-minm)+(m-1)*(n)*(maxm-vec[0]);
        //  cout<<ans1<<" "<<ans2<<nn;
