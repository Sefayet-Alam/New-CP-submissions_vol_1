...
int main()
{
    ...
    //   cout<<ls<<nn<<rs<<nn;
    for(ll i=0;i<n;i++){
        ranges.push_back({ls[i],rs[i]});
    }
    sort(all(ranges));
    ll tot=0;
    for(ll i=0;i<n;i++){
        tot+=ranges[i].second-ranges[i].first+1;
    }
    if(tot<k){
        cout<<-1<<nn;
        continue;
    }
    ll l,r,szon=0,sznoton=0,non=0,need=0;
    ll ans=M;
    ll totinc=0;
    for(ll i=0;i<n;i++){
        l =ranges[i].first;
        ...
        if(sznoton+curr>=k){
            need=k-sznoton;
            ans=min(ans,l+need+1+non*2);
        }
        else if(totinc>=k){
          if(sznoton+curr<=k){
            need=k-(sznoton+curr);
            ans=min(ans,r+2LL+2*non+2*need);
          }
        }
        if(l==r){
            szon++;
