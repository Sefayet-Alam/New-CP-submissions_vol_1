...
ll getMin(ll index){
    ll ret = INT_MAX; // Iniialize result
    // Traverse ancestors of BITree[index]
    while (index>0){  
        ret = min(ret,BITree[index]);  // Add current element of BITree to sum
...
void updateBIT(ll n, ll index, ll val){
    ...
    while (index <= n){
       ...
       BITree[index] = min(val,BITree[index]);
...
int main()
{
    ...
    //   }
      sort(all(vec),cmp);
    //   cout<<vec<<nn;
      fill(BITree,BITree+n+1,INT_MAX);
      ll ans=0;
      for(ll i=0;i<n;i++){
        ll k=getMin(vec[i].second.first);
        if(k>vec[i].second.second){
            ans++;
        }
        updateBIT(n+1,vec[i].second.first,vec[i].second.second);
      }
      cout<<ans<<nn;
    }
