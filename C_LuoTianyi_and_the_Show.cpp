...
ll n,m;
ll func(vector<ll>sitvis,ll on,ll tw,ll minm,ll maxm){
      ll min2=minm;
      ll max2=maxm;
      ll ans=0;
    //   cout<<minm<<" "<<maxm<<nn;
      for(ll i=minm+1;i<=maxm;i++){
        if(sitvis[i]){minm=i;}
        else if(tw){
            // ans++;
            tw--;
...
int main()
{
    ...
    for(auto it:st){
     ...
     ll sz=pos.size();
     ll ans=min(m,sz+max(on,tw));
     for(ll i=0;i<sz;i++){
        ans=max(ans,sz+min(on,pos[i]-1-i)+min(tw,m-pos[i]-(sz-i-1)));
     }
     cout<<ans<<nn;
    }
