...
int main()
{
	...
	while(t--){
        ...
        for(ll i=0;i<n;i++){
            for(ll j=1;j<4;j++){
                cnt[i][j]=digitsum(cnt[i][j-1]);
            }
        }
		ll temp;
		for(ll i=0;i<n;i++){
			temp=queryLazySegTree(0,0,n-1,i,i);
			updateLazySegTree(0,0,n-1,i,i,-temp);
		}
		//cout<<vec<<endl;
		ll x;
		while(k--){
			...
			if(x==1){
				ll l,r;
				cin>>l>>r;
                updateLazySegTree(0,0,n-1,l-1,r-1,1);
			}
			else{
				cin>>x;
                x--;
                temp=queryLazySegTree(0,0,n-1,x,x);
                if(temp>3) temp=3;
