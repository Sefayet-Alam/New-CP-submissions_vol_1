...
void func(ll n){
   ...
   QP<pair<ll,pair<ll,ll> >>ranges;

    ranges.push({-n,{0,n-1}});

    while(ranges.size()){

     ll low=ranges.top().second.first;

     ll up=ranges.top().second.second;
     ...
     //cout<<low<<" "<<mid<<" "<<up<<" "<<ans[mid]<<" "<<endl;
     k++;

     if(mid-low>0) ranges.push({-(mid-low),{low,mid-1}});
     if(up-mid>0) ranges.push({-(up-mid),{mid+1,up}});
