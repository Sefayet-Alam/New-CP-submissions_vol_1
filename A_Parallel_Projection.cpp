...
int main()
{
    ...
    while(t--){
        ...
        ll a,b,f,g;
        cin>>a>>b>>f>>g;
        ll dist1=h+g+b+abs(a-f);
        ll dist2=h+abs(d-g)+abs(d-b)+abs(a-f);
        ll dist3=h+f+a+abs(b-g);
        ll dist4=h+abs(w-f)+abs(w-a)+abs(b-g);
        ll ans1=min(dist1,dist2);
         ll ans2=min(dist3,dist4);
