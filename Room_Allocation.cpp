...
int main()
{
   ...
   // cin>>t;
    ...
    //    }
       ...
       //cout<<vec[0].second<<" "<<alloc[vec[0].second]<<endl;
       priority_queue<pair<ll,ll>>pq;
       for(ll i=0;i<n;i++){
        if(pq.empty()){
            roomno++;
            pq.push({-vec[i].first.second,roomno});
			alloc[vec[i].second] = roomno;
        }
        else {
			// accessing the minimum departure time
			pair<int, int> minimum = pq.top();
			if (-minimum.first < vec[i].first.first) {
				pq.pop();
				pq.push({-vec[i].first.second, minimum.second});
				alloc[vec[i].second] = minimum.second;
			}
