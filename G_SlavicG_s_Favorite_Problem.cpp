...
vpll g[N];
ll n,a,b;
unordered_set<ll,custom_hash> XOR;
bool OK=0;
 
void dfs(int vertex,int par,ll x){
    /*
    ...
    */
    ...
    for(auto child: g[vertex]){
        ...
        */
        if(child.first==par||child.ff==b) continue;
        dfs(child.first,vertex,x^child.ss);
        /*
...
void dfs2(int vertex,int par,ll x){
    /*
    take action on vertex after entering the vertex
    */
    for(auto child: g[vertex]){
        ...
        */
        ...
        if(XOR.count(x^child.ss)) OK=1;
        dfs2(child.first,vertex,x^child.ss);
        /*
        take action on child after entering the child node
        */
    }
...
void solve() {
    ll i=0,j=0,k=0,l=0,r=0,ans=0,temp=0,cnt=0,sum=0;
    string s,s1,s2,s3;
    cin>>n>>a>>b;
    fr(i,n+1){
        g[i].clear();
