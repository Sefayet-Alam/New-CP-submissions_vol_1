...
void updateLazySegTree(ll treeIndex, ll lo, ll hi, ll i, ll j, ll val){
     if (lazy2[treeIndex] != 0) {                             // this node is lazy
        tre[treeIndex] =  (tre[treeIndex] *lazy2[treeIndex])%MOD; // normalize current node by removing laziness
 
        if (lo != hi) {                                     // update lazy[] for children nodes
            if(lazy2[2 * treeIndex + 1]==0) lazy2[2 * treeIndex + 1] = lazy2[treeIndex];
            else{
                lazy2[2 * treeIndex + 1] =  (lazy2[2 * treeIndex + 1]*lazy2[treeIndex])%MOD;
            }
            if(lazy2[2 * treeIndex + 2]==0) lazy2[2 * treeIndex + 2] = lazy2[treeIndex];
            else{
                 lazy2[2 * treeIndex + 2] =  (lazy2[2 * treeIndex + 2]*lazy2[treeIndex])%MOD;
            }
        }
 
        lazy2[treeIndex] = 0;                                // current node processed. No longer lazy
    }

    if (lazy[treeIndex] != 0) {                             // this node is lazy
        tre[treeIndex] = (tre[treeIndex] +((hi - lo + 1) * lazy[treeIndex])%MOD)%MOD; // normalize current node by removing laziness
 
        if (lo != hi) {                                     // update lazy[] for children nodes
            lazy[2 * treeIndex + 1] = (lazy[2 * treeIndex + 1]+lazy[treeIndex])%MOD;
            lazy[2 * treeIndex + 2] = (lazy[2 * treeIndex + 2] + lazy[treeIndex])%MOD;
        }
    ...
    if (i <= lo && hi <= j) {                               // segment is fully within update range
        tre[treeIndex] = (tre[treeIndex]+((hi - lo + 1) * val)%MOD)%MOD;             // update segment
 
        if (lo != hi) {                                     // update lazy[] for children
            lazy[2 * treeIndex + 1] = (lazy[2 * treeIndex + 1] +val)%MOD;
            lazy[2 * treeIndex + 2] =   (lazy[2 * treeIndex + 2]+val)%MOD;
        }
    ...
    // merge updates
    tre[treeIndex] = (tre[2 * treeIndex + 1] + tre[2 * treeIndex + 2])%MOD;
}
...
ll queryLazySegTree(ll treeIndex, ll lo, ll hi, ll i, ll j){
    ...
    if (lo > j || hi < i)                                   // segment completely outside range
        ...
        if (lazy2[treeIndex] != 0) {                             // this node is lazy
        tre[treeIndex] *= lazy2[treeIndex]; // normalize current node by removing laziness
    ...
    if (lazy[treeIndex] != 0) {                             // this node is lazy
        tre[treeIndex] =  (tre[treeIndex] +(hi - lo + 1) * lazy[treeIndex])%MOD; // normalize current node by removing laziness
...
void updateLazySegTree2(ll treeIndex, ll lo, ll hi, ll i, ll j, ll val){
     if (lazy[treeIndex] != 0) {                             // this node is lazy
        tre[treeIndex] = (tre[treeIndex] +((hi - lo + 1) * lazy[treeIndex])%MOD)%MOD; // normalize current node by removing laziness
    ...
    if (i <= lo && hi <= j) {  
        if(lazy2[treeIndex]==0)lazy2[treeIndex]=val; 
        else{
            lazy2[treeIndex]=(lazy2[treeIndex]*val)%MOD; 
        }                            // segment is fully within update range
        tre[treeIndex] =  (tre[treeIndex]*lazy2[treeIndex])%MOD;             // update segment
 
        if (lo != hi) {                                     // update lazy[] for children
           if( lazy2[2 * treeIndex + 1]==0)   lazy2[2 * treeIndex + 1] = val;
           else{
             lazy2[2 * treeIndex + 1] =   (lazy2[2 * treeIndex + 1]*val)%MOD;
           }
           if(lazy2[2 * treeIndex + 2]==0)  lazy2[2 * treeIndex + 2] = val;
           else{
             lazy2[2 * treeIndex + 2] =  (lazy2[2 * treeIndex + 2]*val)%MOD;
           }
        }
        lazy2[treeIndex]=0;
    ...
    updateLazySegTree2(2 * treeIndex + 1, lo, mid, i, j, val);
    updateLazySegTree2(2 * treeIndex + 2, mid + 1, hi, i, j, val);
...
int main()
{
    ...
    while(t--){
        ...
        // }
        for(ll i=0;i<3*n;i++){
                //cout<<tre[i]<<" "<<lazy2[i]<<" "<<lazy[i]<<endl;
           }
        while(q--){
            cin>>x;
            if(x==0){
                ...
                updateLazySegTree2(0,0,n-1,l,r-1,b);

                updateLazySegTree(0,0,n-1,l,r-1,c);
                ...
                cout<<queryLazySegTree(0,0,n-1,l,r-1)<<endl;
