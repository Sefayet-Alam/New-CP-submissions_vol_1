...
ll maxmize(ll a,ll b){
    ...
    p=o+p;
    for(ll i=n-1;i>=0;i++){
        if(s[i]=='9' && p[i]!='9') return a;
        if(p[i]=='9' && s[i]!='9') return b;
    }
    }
    if(cnt1>cnt2) return a;
    else if(cnt2>cnt1) return b;
    else return max(a,b);
}
    ...
    while(t--){
     ...
     ll power=1;
       for(ll i=n-1;i>=0;i--){
        ...
        tominus=(s[i]-'0')*tem+1+prev;
        prev+=tem*(s[i]-'0');
        tem*=10;
      
    
        if(end-tominus>=st){
         //    if(ans!=end-tominus) ans=maxmize(ans,end-tominus);
          ans=end-tominus;
        }
        else break;
       }
       cout<<maxmize(end,ans)<<endl;
