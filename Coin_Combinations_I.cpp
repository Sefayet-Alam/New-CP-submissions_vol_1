...
ll coinchange(vector<ll>& a, ll w, ll i)
{
    if (w == 0)
        return dp[i][w] = 1;
    if (i == 0)
    ...
    if (a[i - 1] <= w) {
        ...
        return dp[i][w] = (coinchange(a, w - a[i - 1], i))%M
                          + (coinchange(a, w, i - 1))%M;
    }
    else // We have no option but to leave this coin
        return dp[i][w] = (coinchange(a, w, i - 1))%M;
}
int main()
{
    fast;
   ...
   //cout<<vec<<endl<<amnts<<endl;
   mem(dp,-1);
  
   ll ans=coinchange(vec,k,n);
   cout<<ans<<endl;
