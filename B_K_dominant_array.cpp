...
void add(ll ind){
    if(freq2[freq[arr[ind]]]) freq2[freq[arr[ind]]]--;
    freq[arr[ind]]++;
    freq2[freq[arr[ind]]]++;
    if(freq[arr[ind]]>c) c=freq[arr[ind]];
}
...
int main()
{
    ...
    for(ll i=0;i<q;i++){
        ...
        // }
        // cout<<c<<nn;
        ans[qs[i].ind]=c*(qs[i].k)-(qs[i].r-qs[i].l+1);
    }
     for(ll i=0;i<q;i++){
        if(ans[i]>=0) cout<<"YES"<<nn;
        else cout<<"NO"<<nn;
    }
    return 0;
}