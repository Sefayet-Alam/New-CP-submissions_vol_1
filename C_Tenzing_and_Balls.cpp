...
int main()
{
    ...
    while(t--){
      ...
      for(ll i=0;i<n;i++){
        minpos[vec[i]]=min(minpos[vec[i]],i);
        maxpos[vec[i]]=max(maxpos[vec[i]],i);
      }
      vector<pair<ll,ll>>alls;
      for(ll i=1;i<=n;i++){
        if(maxpos[i]!=-1 && minpos[i]!=maxpos[i]){
            alls.push_back({minpos[i],maxpos[i]});
        }
      }
      ll minm=n+100,maxm=-1;
    ...
    //   }
      ...
      for(ll i=0;i<sz;i++){
        if(i==0){
         ans+=alls[i].second-alls[i].first+1;
         }
        else{
         if(alls[i].first>alls[i-1].second){
            ans+=alls[i].second-alls[i].first+1;
         }
       }
      }
