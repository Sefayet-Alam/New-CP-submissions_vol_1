...
int main()
{
   ...
   if(n>=7){
    ll l=0,r=l+7;
    vector<ll>prefc(n,0),prefa(n,0),prefb(n,0);
    for(ll i=0;i<n;i++){
        if(s[i]=='a') prefa[i]++;
        else if(s[i]=='b') prefb[i]++;
        else if(s[i]=='c') prefc[i]++;
    }   
    for(ll i=1;i<n;i++){
        prefa[i]+=prefa[i-1];
        prefb[i]+=prefb[i-1];
    ...
    if(prefa[6]>prefb[6] && prefa[6]>prefc[6]){
         len=min(len,7LL);
    }
    for(ll i=0;i<n-7;i++){
        ll as=prefa[i+7]-prefa[i];
        ll bs=prefb[i+7]-prefb[i];
        ll cs=prefc[i+7]-prefc[i];
        if(as>bs && as>cs){
            len=min(len,7LL);
        }
    }
