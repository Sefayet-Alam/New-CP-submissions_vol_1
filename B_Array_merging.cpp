...
int main()
{
    ...
    while(t--){
     ...
     for(ll i=0;i<n-1;i++){
        if(vec[i]==vec[i+1]){
            curr++;
            freq[vec[i]]=max(freq[vec[i]],curr);
        }
        else{
            curr=1;
            freq[vec[i]]=max(freq[vec[i]],curr);
      ...
      freq[vec[n-1]]=max(freq[vec[n-1]],curr);
      curr=1;
      ...
      for(ll i=0;i<n-1;i++){
        if(vec2[i]==vec2[i+1]){
            ...
            freq2[vec2[i]]=max(freq2[vec2[i]],curr);
        }
        else{
            curr=1;
            freq2[vec2[i]]=max(freq2[vec2[i]],curr);
        ...
        freq2[vec2[n-1]]=max(freq2[vec2[n-1]],curr);
        ll maxm=-1;
        for(ll i=0;i<n;i++){
            maxm=max(maxm,freq[vec[i]]+freq2[vec[i]]);
        }
         for(ll i=0;i<n;i++){
            maxm=max(maxm,freq[vec2[i]]+freq2[vec2[i]]);
        }
        cout<<maxm<<nn;
