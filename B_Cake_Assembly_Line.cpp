...
int main()
{
    ...
    while(t--){
        ...
        sort(a.begin(),a.end());
        sort(b.begin(),b.end());
        vector<pair<ll,ll> >pa,pb1;
        for(ll i=0;i<n;i++){
            pa.push_back({a[i]-w,a[i]+w});
             pb1.push_back({b[i]-h,b[i]+h});

        }
        ll maxN=0;
        vector<pair<ll,ll>>diffa;
        for(ll i=0;i<n;i++){
            // if(pa[i].first>=pb1[i].first){ 
                diffa.push_back({pa[i].first-pb1[i].first,pa[i].second-pb1[i].second});
        
            // }
            // else{
            //      diffa.push_back({abs(pa[i].second-pb1[i].second),abs(pa[i].first-pb1[i].first)});
                
            // }
        }
        bool f=0;
       
        if(kOverlap(diffa, n)) cout<<"YES"<<endl;
        else cout<<"NO"<<endl;
    }
