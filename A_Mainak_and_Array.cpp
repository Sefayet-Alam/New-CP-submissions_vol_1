...
int main()
{
    ...
    vector<ll>vec(n);
    cin>>vec;
   ll maxm=*max_element(vec.begin(),vec.end());
   ll minm=*min_element(vec.begin(),vec.end());
   if(maxm==vec[n-1] || minm==vec[0]){
    cout<<maxm-minm<<endl;
   }
   else{
    bool f=0;
    for(ll i=0;i<n-1;i++){
        if(vec[i]==maxm && vec[i+1]==minm){
            cout<<maxm-minm<<endl;
            f=1;
            break;
        }
    ...
    ll maxdiff=0;
    for(ll i=0;i<n-1;i++){
        maxdiff=max(maxdiff,vec[i]-vec[i+1]);
    }
    ll ans=max(maxm-vec[0],vec[n-1]-minm);
    cout<<max(ans,maxdiff)<<endl;
   }
    }
