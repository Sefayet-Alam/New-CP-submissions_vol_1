...
ll dp[N];
ll ans;
ll func(ll i,ll maxa,ll maxb){
    //cout<<i<<" "<<maxa<<" "<<maxb<<endl;
    if(i==n){
       if(maxa==0 || maxb==0) return M;
       return dp[i]=abs(maxa-maxb);
    }
    ...
    ll m1=max(maxa,a[i]);
    ll m2=max(maxb,b[i]);
    ans=min(ans,func(i+1,maxa,maxb));

    ans=min(ans,func(i+1,m1,m2));
