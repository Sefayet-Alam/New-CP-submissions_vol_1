...
int main()
{
    ...
    while(t--){
        ...
        multiset<ll>ms;
        for(ll i=0;i<k;i++){//n=k
            ms.insert(vec[i]);
        }
        auto medidx=ms.begin();
        ll idx=0;
        for(ll i=0;i<k/2;i++){//n=k
            medidx++;
            idx++;
        }
        if(k%2==0){medidx--;idx--;}//n=k
        // cout<<idx<<nn;
        ll l=0;
        ll r=k-1;
        ...
        while(r<n-1){
          ...
          if(vec[l]==*medidx){
            ...
            auto it=ms.find(vec[l]);
            ms.erase(it);
            if(vec[l]>*medidx && vec[r]<*medidx){
                medidx--;
            }
            else if(vec[l]<*medidx && vec[r]>=*medidx){
                medidx++;
            }
