...
int main()
{
    ...
    while(t--){
      ...
      d.push_back(cs[0]);
       sort(all(d));
    //   ll maxa=*(max_element(all(as)));
    //   ll maxb=*(max_element(all(bs)));
    //   ll maxc=*(max_element(all(cs)));
     
      ll minm=min({suma,sumb,sumc});
      minm=min(minm,(d[0]+d[1]));
