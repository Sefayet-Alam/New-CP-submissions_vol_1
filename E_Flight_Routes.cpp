...
void dijkstra(ll source){
    ...
    while(pq.size()){
        ...
        pq.pop();
        if(dist[v][k-1]<v_dist) continue;
        for(auto &child:g[v]){
            ll child_v=child.first;
            ll wt=child.second;
            if(dist[child_v][k-1]>v_dist+wt){
                dist[child_v][k-1]=v_dist+wt;
                pq.push({dist[child_v][k-1],child_v}); 
                sort(dist[child_v].begin(),dist[child_v].end());
