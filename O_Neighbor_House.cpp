...
ll dp[N][4];
ll func(ll i,ll colr){
    if(i==n){
        return 0;
    }

    if(dp[i][colr]!=-1) return dp[i][colr];

    ll ans=INT_MAX;

    if(colr!=1) ans=min(ans,func(i+1,1)+a[i]);
    if(colr!=2) ans=min(ans,func(i+1,2)+b[i]);
    if(colr!=3) ans=min(ans,func(i+1,3)+c[i]);

    return dp[i][colr]=ans;
}
int main()
{
    fast;
