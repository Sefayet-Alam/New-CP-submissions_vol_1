...
void CountSort(vector<ll> &vec){
    ll n=vec.size(),mxm=*max_element(all(vec));
    vector<ll> cnt(mxm+1,0);
    vector<ll> sorted(n);
    for(ll i=0;i<n;i++) cnt[vec[i]]++;
    for(ll i=1;i<cnt.size();i++) cnt[i]+=cnt[i-1];
    // cout<<cnt<<endl;
    for(ll i=n-1;i>=0;i--) {sorted[--cnt[vec[i]]]=vec[i];} 
    for(ll i=0;i<n;i++) vec[i]=sorted[i];
}
