...
int main()
{
    ...
    while(t--){
      ...
      map<ll,ll>vis;
      for(ll i=1;i<n-1;i++){
        if(vec[i-1]>vec[i] && vec[i]<vec[i+1]){tot++;vis[i]=2;}
        else if(vec[i-1]<vec[i] && vec[i]>vec[i+1]){tot++;vis[i]=1;}
        else vis[i]=0;
      }
      ans=tot;
      ll maxminus=1;
      for(ll i=1;i<n-1;i++){
       ...
       if(i-1){
        if(vec[i-1]>vec[i-2] && vec[i-1]>vec[i+1]){
            if(vis[i-1]==0) x--;
        }
        else if(vec[i-1]<vec[i-2] && vec[i-1]<vec[i+1])
            {
                if(vis[i-1]==0) x--;
            }
            else
