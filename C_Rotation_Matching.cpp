...
int main()
{
    ...
    vector<ll>diff(n+1);
    for(ll i=1;i<=n;i++){
        //cout<<i<<" "<<posb[i]-posa[i]<<" "<<posb[i]+n-posa[i]<<endl;
        if(posa[i]<=posb[i]) diff[i]=posb[i]-posa[i];
        else{diff[i]=posb[i]+n-posa[i];}
        // diff[i]=max(posb[i]-posa[i],posb[i]+n-posa[i]);
        //cout<<i<<" "<<posa[i]<<" "<<posb[i]<<" "<<diff[i]<<endl;
    }
    map<ll,ll>freq;
    for(ll i=1;i<=n;i++){
        //cout<<diff[i]<<endl;
