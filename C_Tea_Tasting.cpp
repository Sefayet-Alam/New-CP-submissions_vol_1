...
int main()
{
    ...
    while(t--){
        ...
        ll x;
        for(ll i=0;i<n;i++){
            if(i==0) x=upper_bound(pref.begin(),pref.end(),a[i])-pref.begin()-1;
            else{
                x=upper_bound(pref.begin(),pref.end(),a[i]+pref[i-1])-pref.begin()-1;
            }
            ...
            if(x==n-1){
                ...
                drink[i]++;
                drink[x+1]--;
                ll extra=a[i]-pref[x]+pref[i-1];
                if(i==0) extra=a[i]-pref[x];

                ans[x+1]+=extra;
            }
        }
        
        for(ll i=1;i<n;i++)drink[i]+=drink[i-1];
        for(ll i=0;i<n;i++){
            ans[i]+=(b[i]*drink[i]);
        }
