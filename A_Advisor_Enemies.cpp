...
int main()
{
    ...
    while(t--){
        ...
        map<string,ll>vis;
        vector<pair<string,string> >vec(m);
       for(ll i=0;i<m;i++){
       cin>>vec[i];
       }
       for(ll i=1;i<m;i++){
        if(vec[i].first!=vec[i-1].second) f=1;
       }
       //if(vec[0].first!=vec[m-1].second) f=1;
        if(f) cout<<"I disagree with the advisor"<<endl;
        else cout<<"No more comedians++"<<endl;
    }
