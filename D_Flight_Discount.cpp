...
void dijkstra(ll source){
    ...
    while(pq.size()){
        ...
        pq.pop();
        if(f==0) if(dist[v]<v_dist) continue;
		if(f==1) if(dist2[v]<v_dist) continue;
        ...
        for(auto &child:g[v]){
			...
			if(f==0){
				if(dist[child_v]>v_dist+wt){
					dist[child_v]=v_dist+wt;
					pq.push({dist[child_v],{child_v,0}});
					//cout<<v<<" "<<child_v<<" "<<f<<" "<<dist[child_v]<<endl;
				}
				if(dist2[child_v]>v_dist+wt/2){
					
					dist2[child_v]=v_dist+wt/2;
					pq.push({dist2[child_v],{child_v,1}});
					//cout<<v<<" "<<child_v<<" "<<f<<" "<<dist2[child_v]<<endl;
				}

			}
			if(f==1){
				if(dist2[child_v]>v_dist+wt){
					dist2[child_v]=v_dist+wt;
					pq.push({dist2[child_v],{child_v,1}});
					//cout<<v<<" "<<child_v<<" "<<f<<" "<<dist2[child_v]<<endl;
				}
			}	
