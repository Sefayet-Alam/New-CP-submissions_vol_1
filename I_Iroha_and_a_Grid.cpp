...
int main()
{
    ...
    //cin>>t;
    ll ans=0;
    vector<ll>factorls(N);
    vector<ll>finv(N);
    factorls[0]=finv[0]=1;
    for(ll i=1;i<N;i++){
      factorls[i]=(factorls[i-1]*i)%M;
      finv[i]=inverseMod(factorls[i],M);
    }
    ...
    while(t--){
        ...
        for(ll i=0;i<h-a;i++){
            ll g=i+b-1;
            ll res=factorls[g]*finv[i]%M*finv[g-i]%M;
            ll g2=(w-1-b)+(h-1-i);
            ll p=w-1-b;
            ll res2=factorls[g2]*finv[p]%M*finv[g2-p]%M;
            res=(res*res2)%M;
            ans=(ans+res)%M;
