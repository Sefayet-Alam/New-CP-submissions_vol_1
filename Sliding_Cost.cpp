...
void ins(long val){ // insert val into sets
	...
	if(a < val){
		...
		if(up.size() > M/2){
            ll movedno=*up.begin();
			low.insert(movedno);slw+=movedno;
			up.erase(up.find(movedno));sup-=movedno;
		}
	}
	else{
		low.insert(val);slw+=val;
		if(low.size() > (M + 1)/2){
            ll movedno=*low.rbegin();
			up.insert(movedno);sup+=movedno;
			low.erase(low.find(movedno));slw-=movedno;
		}
	}
}
...
void er(long val){ // erase from sets
	...
	else {low.erase(low.find(val));slw-=val;}
	if(low.empty()){
         ll movedno=*up.begin();
		low.insert(*up.begin());slw+=movedno;
		up.erase(up.find(*up.begin()));sup-=movedno;
	}
}
...
int main()
{
	...
	for(long i = M; i < N; i++){
		...
		//cout << *low.rbegin() << " ";
         cout << sup- slw + med(); if(i != N -1) cout << " ";
	}
	//cout << endl;  
