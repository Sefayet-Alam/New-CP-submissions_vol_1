...
int main()
{
    ...
    if(m<=n){
      ll minm=INT_MAX;
      for(auto it:factofm){
       // cout<<it.first<<" "<<factofn[it.first]<<" "<<it.second<<endl;
       ll g;
       if(!factofn[it.first]) g=0;
       else g=(factofn[it.first])/(it.second);
       minm=min(minm,g);
      }
      
      for(auto it:factofm){
        if(factofn[it.first]) factofn[it.first]-=((it.second)*minm);
      }
     }
      ll ans=1;
      for(auto it:factofn){
        ans=ans*(power(it.first,it.second,c))%c;
      }
     // cout<<ans<<endl;
     ans=ans%c;
      cout<<ans<<endl;
