...
///  n(a1 U a2 U a3)=n(a1)+n(a2)+n(a3)-n(a1 ^ a2) - n(a2 ^ a3) ...+(a1^a2^a3)
...
int main()
{
    ...
    while(t--){
        ...
        ll prod=1;
        //cout<<vec<<endl;
        //(1<<k) means 2^k...there r 2^k-1 combinations in n(a1 U a2 U...)
        for(ll i=1;i<(1<<k);i++){
            ll temp=n;
            ll cntsets=0;
            for(ll j=0;j<k;j++){
                //now we need to find the setbit...i th bit is set?
                if(i&(1<<j)){
                    cntsets++;
                    temp/=vec[j];
                }
            }
            if(cntsets%2) ans+=temp;
            else ans-=temp;
        }
        cout<<ans<<endl;
    }
