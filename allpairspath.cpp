...
int main()
{
    ...
    while(1)
    {
     ...
     vector<ll>answers(q);
     for(ll i=0;i<n;i++){
        ll g=bellman_ford(n,i);
        for(auto it:queries[i]){
            ll k=it.first;
           if(g==-1) answers[it.second]=dist[k];
           else answers[it.second]=M;
        }
     }

      for(auto it:answers){
        if(it==INF) cout<<"Impossible"<<nn;
        else if(it==M) cout<<"-Infinity"<<nn;
        else cout<<it<<nn;
      }
       cout<<nn;
        for(ll i=0;i<=n+1;i++){
